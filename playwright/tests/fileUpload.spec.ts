...
test.describe("File Upload UI Suite", () => {
    ...
    test("test initial state", async ({ page }) => {
        await expect(page.locator('label')).toContainText('Pick anything reasonably sized');
        await expect(page.getByRole('button', { name: 'Submit' })).toBeAttached();
    });

    test('file upload test', async ({ page }) => {
        const basePath = process.cwd().includes('/playwright') ? process.cwd() : path.join(process.cwd(), 'playwright');
        ...
        await expect(page.getByRole('caption')).toContainText('File Upload Results');
        ...
        await expect(page.locator('#file_upload')).toContainText('SHA256 Hash');
        await expect(page.locator('#file_upload')).toContainText('94ee059335e587e501cc4bf90613e0814f00a7b08bc7c648fd865a2af6a22cc2');
    });
});
