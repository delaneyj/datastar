...
func setupExamples(router chi.Router) (err error) {
	...
	examples := lo.Map([]ExampleGroup{
		{
			...
			Examples: []*Example{
				...
				{Label: "Is Fetching", Description: "use an isFetching set of identifiers in a signal to reflect when an element is fetching"},
	...
	router.Route("/examples", func(examplesRouter chi.Router) {
		...
		if err := errors.Join(
			...
			setupExamplesIsFetching(examplesRouter),
