...
test.describe("Todos UI Suite", () => {
    ...
    test('test initial state', async ({ page }) => {
        ...
        await expect(page.locator("#todo0")).toContainText('Learn any backend language');
    ...
    test('test todos modes', async ({ page }) => {
        ...
        await active.click();
        await expect(page.locator("#todo0")).not.toBeAttached();
        await expect(page.locator("#todo1")).toContainText('Learn Datastar');
        await expect(page.locator("#todo2")).toContainText('???');
        await expect(page.locator("#todo3")).toContainText('Profit');
        await completed.click();
        await expect(page.locator("#todo0")).toContainText('Learn any backend language');
        await expect(page.locator("#todo1")).not.toBeAttached();
        await expect(page.locator("#todo2")).not.toBeAttached();
        await expect(page.locator("#todo3")).not.toBeAttached();
        await all.click();
        await expect(page.locator("#todo0")).toContainText('Learn any backend language');
        await expect(page.locator("#todo1")).toContainText('Learn Datastar');
        await expect(page.locator("#todo2")).toContainText('???');
