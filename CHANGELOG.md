...
- Changed the `data-*` attribute modifier delimiter from `.` to `_` for modifiers and from `_` to `.` for arguments.  This is to be spec compliant while still parseable with new nested signal syntax #345 (`data-on-keydown_debounce.100ms_throttle.lead="value"`).
...
- Nested signals no longer allow for `_` in the key.  It causes a conflict with modifiers.
